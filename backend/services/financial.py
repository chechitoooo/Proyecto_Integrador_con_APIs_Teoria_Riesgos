...
def calcular_var(ticker, confianza, inversion, n_sims):
    ...
    if T == 0:
        kupiec_ok, lr_uc, p_valor, exc_esp = True, 0.0, 1.0, 0.0
    else:
        ...
        kupiec_ok = bool(p_valor > 0.05)
        exc_esp = round(T * p_esperada, 1)
    ...
    return {
        ...
        "excedencias_kupiec": excedencias,
        "excedencias_esperadas_kupiec": round(exc_esp, 1),
        "lr_uc_kupiec": round(float(lr_uc), 4),
        "p_valor_kupiec": round(p_valor, 4),
        "aprueba_kupiec": kupiec_ok,
    }
...
def calcular_bono(cupon_pct, vencimiento, valor_nominal, frecuencia):
    ...
    convex = sum(
        (i / frecuencia) * ((i / frecuencia) + 1 / frecuencia) * (cf / (1 + ytm_periodo) ** i)
        for i, cf in enumerate(flujos, 1)
    ) / (precio * (1 + ytm_periodo) ** 2)
    return {
        "precio": round(precio, 4), "ytm_pct": round(ytm * 100, 4),
...
def calcular_opciones(ticker, strike, vencimiento_dias, tasa_libre_riesgo):
    ...
    d1 = (np.log(S / strike) + (tasa_libre_riesgo + 0.5 * sigma ** 2) * T) / (sigma * np.sqrt(T))
    d2 = d1 - sigma * np.sqrt(T)
    call_price = S * norm.cdf(d1) - strike * np.exp(-tasa_libre_riesgo * T) * norm.cdf(d2)
    put_price = strike * np.exp(-tasa_libre_riesgo * T) * norm.cdf(-d2) - S * norm.cdf(-d1)
    ...
    theta_call = float((-(S * norm.pdf(d1) * sigma) / (2 * np.sqrt(T)) - tasa_libre_riesgo * strike * np.exp(-tasa_libre_riesgo * T) * norm.cdf(d2)) / 365)
    rho_call = float(strike * T * np.exp(-tasa_libre_riesgo * T) * norm.cdf(d2) / 100)
    return {
        "ticker": ticker, "precio_spot": round(S, 4), "strike": strike,
        "sigma": round(sigma, 4), "T_anios": round(T, 4),
